...
miRNA_to_genesFile_new <- synGet('syn3461627', version=1)
miRNA_to_genes_new <- read.delim(miRNA_to_genesFile_new@filePath,
                                 col.names=c("mirName", "GeneID"),
                                 header=T, sep="\t",
                                 stringsAsFactors=FALSE, check.names=F)

miRNA_to_genes_new %>%
  count(mirName) %>%
  summarize(mean_degree=mean(n))
